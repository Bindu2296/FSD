$(document).ready(()=>{
 ...
 //$('#valite').html(`<table><tr><td>Name</td><td>${x}</td></tr><tr><td>DOB</td><td>${y}</td></tr><tr><td>Address</td><td>${y1}</td></tr><tr><td>DL No:</td><td>${z}</td></tr>`);
  ...
  for(var i=0;i<localStorage.length;i++)
  {
    cards=cards +
          `<div class="card" id='card' style='width: 50em'>
          <div class="card-header" align="center">Your Info <button type="button" class="btn btn-primary" onclick"">Edit</button><button type="button" class="btn btn-primary" onclick"onClarPressed()">Delete</button></div>
      <div class="card-body">  <div  class="row">
        <div class="col-md-4">
  <img class="card-img-top img-fluid" src="Tulips.jpg" /></div>
          <div id="valite" class="card-text">
  <table><tr><td>Name</td><td>${x}</td></tr><tr><td>DOB</td><td>${y}</td></tr><tr><td>Address</td><td>${y1}</td></tr><tr><td>DL No:</td><td>${z}</td></tr>
 </table> </div></div>
  </div>
</div> `};  
